...
/*
 *   @(#)InstrumentationAttach.test.xml 08/01/31 Yuri Gaevsky, Dmitry Miltsov
...
import javasoft.sqe.jck.lib.attach.AttachConnector.AttachException;
import javasoft.sqe.jck.lib.attach.AttachConnector;
import javasoft.sqe.jck.lib.attach.JPLISAttachConnector;
import javasoft.sqe.jck.lib.attach.JVMTIAttachConnector;


public class InstrumentationAttachTests extends ReadTest {

    public AttachConnector connector = null;

    public String agent1 = null;
    public String agent1opts = null;
    public String agent2 = null;
    public String agent2opts = null;

    /**
     * The string representation of name for an implementation
     * of AttachConnector class.
     *
     * @see init
     */
    protected String attachClassName = null;

    /**
     * Checks whether the test specific arguments are supplied.
     *
     * @exception SetupException raised when arguments
     *           are not specified.
     */
    protected void init() throws SetupException {
        super.init();
        if (attachClassName == null || "".equals(attachClassName)) {
            throw new SetupException( "No -attachClassName specified" );
        }

        if (agent1 == null || "".equals(agent1)) {
            throw new SetupException( "No -agent1 specified" );
        }

        if (agent1opts == null || "".equals(agent1opts)) {
            throw new SetupException( "No -agent1opts specified" );
        }

        if (agent2 == null || "".equals(agent2)) {
            throw new SetupException( "No -agent2 specified" );
        }

        if (agent2opts == null || "".equals(agent2opts)) {
            throw new SetupException( "No -agent2opts specified" );
        }

        try {
           connector = (AttachConnector) Class.forName(attachClassName).newInstance();
        } catch (Throwable exc) {
            exc.printStackTrace(ref);
            throw new SetupException(
                    "Cannot create an instance of " + attachClassName
                  + " class supplied for -attachClassName,"
                  + " unexpected exception thrown: " + exc);
        }
        ...
        try {
            ...
            try {
                connector.attach(id);
                ref.println("Successfully attached to JVM with id=" + id);

                try {
                   connector.loadAgent(agent1, agent1opts);
                   ref.println("Agent successfully loaded:  \n\tname=" + agent1
                             + "\n\topts=" + agent1opts);
                } catch (AttachException exc) {
                   ref.println("Loading of agent1 failed,"
                             + " unexpected exception thrown: " + exc);
                   exc.printStackTrace(ref);
                }

                try {
                   connector.loadAgent(agent2, agent2opts);
                   ref.println("Agent successfully loaded: \n\tname=" + agent2
                             + "\n\topts=" + agent2opts);
                } catch (AttachException exc) {
                   ref.println("Loading of agent2 failed,"
                             + " unexpected exception thrown: " + exc);
                   exc.printStackTrace(ref);
                }

            } catch (AttachException exc) {
                ref.println("Cannot attach to JVM with id=" + id + " ,"
                         + " unexpected exception thrown: " + exc);
                exc.printStackTrace(ref);
            }
        } catch (Throwable exc) {
            ref.println("Cannot get JVM id():\n" + exc);
            exc.printStackTrace(ref);
        }
    }

    /**
     * Decodes test-specific arguments.
     *
     * @param   argv
     *          execute arguments from the test harness or from the command line.
     * @param   index
     *          current index into argv.
     * @return  number of arguments decoded if test specific argument is
     *          recognized, or <CODE>super.decodeArg(argv, index)</CODE>
     *          otherwise.
     * @exception SetupException raised when an invalid arguments is passed, or
     *          another error occurred.
     */
    public int decodeArg(String[] args, int index) throws SetupException {
        if (args[index].equals("-attachClassName")) {
            if (index+1 < args.length) {
                attachClassName = args[++index];
                return 2;
            } else {
                throw new SetupException("No value supplied for "
                        + "-attachClassName");
            }
        } else if (args[index].equals("-agent1")) {
            if (index+1 < args.length) {
                agent1 = args[++index];
                return 2;
            } else {
                throw new SetupException("No value supplied for -agent1");
            }
        } else if (args[index].equals("-agent1opts")) {
            if (index+1 < args.length) {
                agent1opts = args[++index];
                return 2;
            } else {
                throw new SetupException("No value supplied for -agent1opts");
            }
        } else if (args[index].equals("-agent2")) {
            if (index+1 < args.length) {
                agent2 = args[++index];
                return 2;
            } else {
                throw new SetupException("No value supplied for -agent2");
            }
        } else if (args[index].equals("-agent2opts")) {
            if (index+1 < args.length) {
                agent2opts = args[++index];
                return 2;
            } else {
                throw new SetupException("No value supplied for -agent2opts");
            }
        } else {
    ...
    public static void main(String argv[]) {
        InstrumentationAttachTests test = new InstrumentationAttachTests();
        test.run(argv, System.err, System.out).exit();
    }

    /**
     * Assertion testing
     * for public static void agentmain(String agentArgs, Instrumentation inst),
    ...
    public Status Instrumentation048() {
        String testCaseID = "Instrumentation048";
            ...
            ClassLoader cl = null;
            try {
                cl = new LocalDirClassLoader(
                        getClassDirURL(), this.getClass().getClassLoader());
            } catch (Exception e){
                return Status.failed("Unable to create LocalDirClassLoader, "
                                   + "exception thrown: " + e);
            }
            ...
            try {
                ...
                return Status.failed("Unable to load class " + classToLoad + ", "
                                   + "exception thrown: " + e);
            }
            ...
            try {
                ...
                return Status.failed("Unable to instantiate class " + classToLoad
                        + ", exception thrown: " + e);
            }

            if (!expectedString.equals(obj.getString())){
                return Status.failed("Unexpected string value: \"" + obj.getString()
                                   + "\", instead of expected: \"" + expectedString
                                   + " in the transformed class ClassP1");
            }
    ...
    /**
     * Assertion testing
     * for boolean isRetransformClassesSupported(),
     * Retransformation will only be supported if the Can-Retransform-Classes
     * manifest attribute is set to true in the agent JAR file (as described
     * in the package specification) and the JVM supports this capability..
     */
     ...
     * <br><b>Expected results</b>: the premain method is not invoked
     */
